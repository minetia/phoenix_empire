...
print("🔥 [Project Phoenix V11] 데이터 로딩 버그 완벽 수정 중...")
...
class PhoenixTrader:
    ...
    async def get_portfolio_status(self):
        ...
        for c in self.port:
            ...
            res.append({
                "name": c["name"], "code": sym, "qty": float(c["qty"]), "avg": float(c["avg"]), 
                "cur_krw": float(cp), "cur_usd": float(cp/usdt), "val": float(val), "prof": float(prof), "rate": float(rate)
            })
        ...
        daily_pnl = []
        now = datetime.now()
        base_asset = 120000000 
        cur_acc_prof = -5000000 
        ...
        for i in range(7, -1, -1):
            ...
            daily_pnl.insert(0, {
                "date": d_str, 
                "d_prof": float(d_prof), "a_prof": float(cur_acc_prof),
                "d_rate": float(d_rate), "a_rate": float(acc_rate),
                "s_asset": float(start_asset), "e_asset": float(end_asset)
            })

        return {
            "usdt": float(usdt), "data": res, "hist": self.hist, 
            "ai_tot": float(tot_ai), "analysis": analysis_data, "daily_pnl": daily_pnl
        }
"""

# HTML (캐시 방지 메타태그 및 렌더링 버그 수정)
...
<head>
    ...
    <title>Phoenix PRO - AI Trading V11</title>
...
<body class="h-screen flex flex-col select-none">
    <header class="h-12 bg-panel border-b border-line flex justify-between items-center px-4 shrink-0">
        <div class="flex items-center space-x-6">
            <h1 class="text-lg font-bold text-white tracking-wider">UPBIT <span class="text-yellow-500 text-sm border border-yellow-500 px-1 rounded ml-1">AI PRO V11</span></h1>
...
def install():
    ...
    for path, content in files.items():
        ...
        print(f"✅ V11 패치 완료: {path}")
    print("\n🎉 모든 버그가 수정되었습니다! 서버를 실행해주세요.")
