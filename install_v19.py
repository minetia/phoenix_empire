...
print("🔥 [Project Phoenix V19] 비동기 엔진 최적화 및 렉(Lag) 제거 패치 중...")

files = {}

# 1. Main Server (비동기 엔진 최적화)
...
async def lifespan(app: FastAPI):
    # 서버 켜질 때 AI 트레이딩 봇 시작
    task = asyncio.create_task(trader.simulate_ai_trading())
    yield
    task.cancel()
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    while True:
        try:
            # V19 핵심: 화면에 데이터 쏘는 주기를 1초로 늦춰 부하 감소
...
# 3. Exchange (V19: 비동기 블로킹 해소를 위해 asyncio.to_thread 적용!!!)
files["core/exchange.py"] = """import pyupbit
import asyncio

class Exchange:
    # 무거운 업비트 API 통신을 별도 쓰레드로 빼서 전체 서버 멈춤 방지
    async def get_current_price_async(self, tickers):
        try:
            res = await asyncio.to_thread(pyupbit.get_current_price, tickers)
...
class PhoenixTrader:
    ...
    async def simulate_ai_trading(self):
        while True:
            # 트레이딩 주기 완화 (렉 방지)
            d = {"safe": 10, "balance": 6, "aggressive": 3}.get(self.ai_mode, 6)
            ...
            # 비동기 시세 조회
            prc_dict = await self.ex.get_current_price_async([c])
    ...
    async def get_portfolio_status(self):
        ...
        # 비동기 대량 시세 조회 (렉의 주범 해결)
        prc = await self.ex.get_current_price_async(tkrs)
        
        # 업비트 응답 실패시 캐시된 가격 사용
        if not prc: 
            prc = self.last_price_cache
        else:
            self.last_price_cache = prc
...
def install():
    ...
    for path, content in files.items():
        ...
        print(f"✅ V19 비동기 엔진 초고속 패치 완료: {path}")
    print("\n🎉 노란 경고창 해결 및 렉(Lag) 유발 교통체증을 완벽하게 뚫었습니다!")
