...
print("🔥 [Project Phoenix V28] 5대잼 랭킹전 & 하이퍼 스피드 엔진 장착 중...")

# [필수] AI 뇌 구조가 완전히 바뀌었으므로 기존 세이브 파일 삭제 (초기화)
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    while True:
        try:
            ...
            break
        await asyncio.sleep(0.5) # 화면 갱신은 0.5초마다 아주 부드럽게!
...
if __name__ == "__main__":
    ...
    print("\\n" + "★"*60)
    print("🦅 [Project Phoenix V28] 하이퍼 스피드 & 랭킹 엔진 가동!")
    print("★"*60)
...
# 4. Trader (V28: 하이퍼 스피드 매매 & 5대잼 개인 지갑/랭킹 시스템 도입!)
...
class PhoenixTrader:
    def __init__(self):
        ...
        # [V28 핵심] 5대잼 고유 지갑 및 실적 데이터베이스
        self.agents = {
            "스캘핑 잼": {"profit": 0.0, "holds": {c["code"]: 0.0 for c in self.port}},
            "추세추종 잼": {"profit": 0.0, "holds": {c["code"]: 0.0 for c in self.port}},
            "방패 잼": {"profit": 0.0, "holds": {c["code"]: 0.0 for c in self.port}},
            "스나이퍼 잼": {"profit": 0.0, "holds": {c["code"]: 0.0 for c in self.port}},
            "고래 잼": {"profit": 0.0, "holds": {c["code"]: 0.0 for c in self.port}}
        }
    ...
    async def simulate_ai_trading(self):
        while True:
            # [V28 핵심] 하이퍼 스피드 매매 속도 (0.5초 ~ 1.5초마다 거래 발생!)
            d = {"safe": 2.0, "balance": 1.0, "aggressive": 0.3}.get(self.ai_mode, 1.0)
            ...
            # AI 성향 로직
            if ai_name == "스캘핑 잼":
                if bot_hold > 0 and profit_rate > 0.5: side = "매도"; sell_ratio = 1.0 
                elif bot_hold > 0 and profit_rate < -1.0: side = "매도"; sell_ratio = 0.5 
                else: side = "매수"; bet_ratio = 0.03
            elif ai_name == "추세추종 잼":
                if profit_rate > 1.5: side = "매수"; bet_ratio = 0.1 
                elif profit_rate < -3.0 and bot_hold > 0: side = "매도"; sell_ratio = 0.8
                else: side = random.choice(["매수", "매도"]); bet_ratio = 0.05; sell_ratio = 0.3
            elif ai_name == "방패 잼":
                if profit_rate < -5.0: side = "매수"; bet_ratio = 0.15 
                elif bot_hold > 0 and profit_rate > 2.0: side = "매도"; sell_ratio = 1.0
                else: continue
            elif ai_name == "스나이퍼 잼":
                if profit_rate < -8.0 or random.random() < 0.05: side = "매수"; bet_ratio = 0.3 
                elif bot_hold > 0 and profit_rate > 5.0: side = "매도"; sell_ratio = 1.0
                else: continue
            elif ai_name == "고래 잼":
                if random.random() < 0.15: 
                    side = random.choice(["매수", "매도"]); bet_ratio = 0.4; sell_ratio = 0.5
                else: continue
            ...
            if side == "매수":
                ...
                # AI 개인 지갑 및 실적 갱신
                self.agents[ai_name]["holds"][c] = bot_hold - qty
                self.agents[ai_name]["profit"] += (sell_amount - buy_amount)
...
# HTML UI (V28: 사라진 투자내역 복구, 랭킹 보드 추가, 차트 속도 최적화)
...
<body class="md:h-screen flex flex-col select-none overflow-y-auto md:overflow-hidden bg-[#0b1016]">
    <header class="py-3 md:h-14 md:py-0 bg-panel border-b border-line flex flex-col md:flex-row justify-between items-center px-4 shrink-0 gap-3">
        <div class="flex flex-col md:flex-row items-center w-full md:w-auto gap-3 overflow-hidden">
            <div class="flex items-center cursor-pointer shrink-0" onclick="setTab('trd')">
                ...
                <span class="text-[10px] text-yellow-500 border border-yellow-500/50 bg-yellow-900/30 px-1.5 py-0.5 rounded ml-2 font-bold tracking-wide">AI CORE V28</span>
            </div>
    ...
    <main id="v-trd" class="flex-1 flex flex-col md:flex-row overflow-y-auto md:overflow-hidden p-1 gap-1">
        ...
        <div class="w-full md:w-[350px] h-[400px] md:h-auto flex flex-col bg-panel shrink-0 mt-1 md:mt-0">
            <div class="flex text-sm border-b border-line bg-[#0b1016]">
                ...
                <div id="t-hist" class="flex-1 text-center py-3 o-hist" onclick="setOrd('history')">⚡ 초스피드 거래내역</div>
            </div>
    ...
    <main id="v-pnl" class="flex-1 hidden bg-[#0b1016] p-2 md:p-4 overflow-y-auto w-full">
        <div class="max-w-[1200px] mx-auto mt-2 md:mt-4">
            ...
            <h2 class="text-xl md:text-2xl font-bold mt-8 md:mt-10 mb-4 text-yellow-500 border-b border-gray-800 pb-2 flex items-center gap-2">🏆 5대잼 수익 랭킹 & 보유 현황</h2>
            <div class="bg-panel rounded-lg border border-gray-800 overflow-x-auto w-full mb-8">
                <div class="flex text-xs text-gray-400 py-3 md:py-4 px-4 md:px-6 border-b border-gray-800 bg-[#0b1016] min-w-[700px]">
                    <div class="w-[20%] font-bold">순위 / AI 잼</div>
                    <div class="w-[30%] text-right">지금까지 번 돈 (실현 손익)</div>
                    <div class="w-[50%] pl-8">누가 어떤 코인을 가지고 있나?</div>
                </div>
                <div id="ai-ranking-list" class="divide-y divide-gray-800 text-xs md:text-sm min-w-[700px]">
                    <div class="p-10 text-center text-gray-500 animate-pulse">AI 실적 정산 중...</div>
                </div>
            </div>

            <h2 class="text-lg md:text-xl font-bold mt-8 md:mt-10 mb-4 text-white border-b border-gray-800 pb-2">📂 AI 코인별 종합 장부</h2>
    ...
    <script>
        ...
        // V28 차트 최적화: 불필요한 툴바 제거하여 더 빠르고 깔끔하게
        function loadChart() {
        ...
        function connectWebSocket() {
            ...
            ws.onmessage = e => { 
                try {
                    ...
                    // V28 픽스: try-catch 내에서 안전하게 나의 투자내역 렌더링
                    ...
                    if(elPtotProf) {
                        ...
                        elPtotProf.className=`text-2xl md:text-3xl font-bold num-font ${tp>=0?'up-red':'down-blue'}`;
                    }
                    const elPlist = document.getElementById('p-list');
                    if(elPlist) elPlist.innerHTML=ph; // 사라졌던 투자내역 복구!
                    ...
                    if(r.ai_coin_pnl) {
                        ...
                        r.ai_coin_pnl.forEach(d => {
                            ...
                            aiPnlHtml += `<div class="flex py-4 px-4 md:px-6 hover:bg-gray-800 transition items-center border-b border-gray-800 min-w-[600px] ${rowCls}">
                    ...
                    if(r.ranking) {
                        ...
                        r.ranking.forEach((bot, idx) => {
                            ...
                            rankHtml += `<div class="flex py-4 px-4 md:px-6 hover:bg-gray-800 transition items-center border-b border-gray-800 min-w-[700px]">
                                <div class="w-[20%] font-bold text-sm md:text-base text-white"><span class="mr-2">${rankIcon}</span>${bot.name}</div>
                                <div class="w-[30%] text-right font-bold num-font text-sm md:text-base ${pCls}">${pSign}${Math.round(p).toLocaleString()} KRW</div>
                                <div class="w-[50%] pl-8 text-xs md:text-sm text-yellow-400 truncate" title="${holdsText}">${holdsText}</div>
                            </div>`;
                        });
