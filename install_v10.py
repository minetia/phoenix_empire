...
print("🔥 [Project Phoenix V10] 투자손익 탭 신설 및 무한로딩 픽스 중...")
...
class PhoenixTrader:
    ...
    async def get_portfolio_status(self):
        ...
        # [V10 추가] 일일 투자손익 모의 데이터 생성 로직
        daily_pnl = []
        now = datetime.now()
        base_asset = 120000000 # 가상의 기초 기준 자산 (1억 2천)
        cur_acc_prof = -5000000 # 가상의 누적 손익 (-500만)
        
        for i in range(7, -1, -1):
            d_str = (now - timedelta(days=i)).strftime("%Y-%m-%d")
            # 오늘 날짜는 실제 현재 변동성 반영, 과거는 랜덤
            d_prof = random.uniform(-1500000, 2500000) if i != 0 else sum([x['prof'] for x in res]) * 0.1
            
            cur_acc_prof += d_prof
            start_asset = base_asset
            base_asset += d_prof
            end_asset = base_asset
            
            d_rate = (d_prof / start_asset) * 100 if start_asset else 0
            acc_rate = (cur_acc_prof / 120000000) * 100
            
            daily_pnl.insert(0, {
                "date": d_str, "d_prof": d_prof, "a_prof": cur_acc_prof,
                "d_rate": d_rate, "a_rate": acc_rate,
                "s_asset": start_asset, "e_asset": end_asset
            })

        return {
            "usdt": usdt, "data": res, "hist": self.hist, 
            "ai_tot": tot_ai, "analysis": analysis_data, "daily_pnl": daily_pnl
        }
"""

# HTML Part 1 (헤더에 투자손익 탭 추가)
html_part1 = """<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8"><title>Phoenix PRO - AI Trading V10</title>
...
<body class="h-screen flex flex-col select-none">
    <header class="h-12 bg-panel border-b border-line flex justify-between items-center px-4 shrink-0">
        <div class="flex items-center space-x-6">
            <h1 class="text-lg font-bold text-white tracking-wider">UPBIT <span class="text-yellow-500 text-sm border border-yellow-500 px-1 rounded ml-1">AI PRO V10</span></h1>
        ...
        <div class="flex-1 flex flex-col space-y-1 overflow-hidden min-w-[600px]">
            ...
            <div class="flex-[2] bg-panel flex">
                ...
                <div class="w-1/2 flex flex-col bg-[#0b1016]">
                    ...
                    <div id="ai-analysis-box" class="flex-1 p-3 space-y-2 text-[11px] overflow-y-auto border-t border-gray-800"><div class="text-center text-gray-500 mt-4 animate-pulse">AI 실시간 분석 중...</div></div>
                </div>
            </div>
        </div>
...
# HTML Part 3 (투자내역 뷰 & 새로 추가된 투자손익 뷰)
html_part3 = """
    ...
    <main id="v-pnl" class="flex-1 hidden bg-[#0b1016] p-4 overflow-y-auto w-full">
        <div class="max-w-[1200px] mx-auto mt-4">
            <h2 class="text-2xl font-bold mb-6 text-white border-b border-gray-800 pb-2 flex items-center gap-2">📊 일일 투자손익 상세 <span class="text-sm font-normal text-gray-500">KRW 환산 추정값</span></h2>
            ...
            <div class="bg-panel rounded-lg border border-gray-800 overflow-hidden w-full">
                <div class="flex text-xs text-gray-400 py-4 px-6 border-b border-gray-800 bg-[#0b1016]">
                    ...
                    <div class="w-[30%] text-right">일일 손익<br>누적 손익</div>
                    <div class="w-[20%] text-right">일일 수익률<br>누적 수익률</div>
                    <div class="w-[30%] text-right">기말 자산<br>기초 자산</div>
                </div>
                <div id="daily-pnl-list" class="divide-y divide-gray-800 text-sm">
                    <div class="p-10 text-center text-gray-500 animate-pulse">데이터 로딩 중...</div>
                </div>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        // 탭 제어 함수 완벽 수정 (3개 탭 대응)
        function setTab(t) { 
            const views = ['trd', 'port', 'pnl'];
            views.forEach(id => {
                const viewEl = document.getElementById('v-'+id);
                const navEl = document.getElementById('n-'+id);
                
                if(t === id) {
                    viewEl.className = id === 'trd' ? "flex-1 flex overflow-hidden p-1 space-x-1" : "flex-1 bg-[#0b1016] p-4 overflow-y-auto w-full";
                    navEl.className = "text-white border-b-2 border-white pb-[14px] pt-4 cursor-pointer";
                } else {
                    viewEl.className = "hidden";
                    navEl.className = "text-gray-500 hover:text-white pb-[14px] pt-4 border-b-2 border-transparent cursor-pointer";
                }
            });
        }
            ...
            try {
                ...
                if(r.daily_pnl) {
                    ...
                    r.daily_pnl.forEach(d => {
                        ...
                        dpHtml += `<div class="flex py-5 px-6 hover:bg-gray-800 transition items-center">
                            <div class="w-[20%] text-gray-300 font-bold">${d.date}</div>
                            <div class="w-[30%] text-right flex flex-col gap-1"><span class="${dc} font-bold num-font text-base">${Math.round(d.d_prof).toLocaleString()}</span><span class="${ac} text-xs num-font">${Math.round(d.a_prof).toLocaleString()}</span></div>
                    ...
                    document.getElementById('daily-pnl-list').innerHTML = dpHtml;
                }
