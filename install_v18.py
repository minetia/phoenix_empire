...
print("🔥 [Project Phoenix V18] 무한 로딩 교통체증(I/O 병목) 완벽 해결 중...")

files = {}

# 1. Main Server (최신 lifespan 방식 적용하여 노란 경고창 제거!)
...
# 최신 파이썬 문법 적용 (DeprecationWarning 해결)
@asynccontextmanager
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    while True:
        try:
            ...
            print(f"❌ [에러 방어 작동] 데이터를 화면에 보내는 중 충돌 발생: {e}")
            traceback.print_exc()
...
# 4. Trader (교통사고 유발하던 Save 로직 분리 완벽 수정)
...
class PhoenixTrader:
    ...
    async def simulate_ai_trading(self):
        while True:
            ...
            self.save_state() # 체결 완료 후 1회만 안전하게 저장
    ...
    async def get_portfolio_status(self):
        ...
        for c in self.port:
            ...
            if ai_qty > 0 and ai_avg_price <= 0:
                ...
                # [버그 핵심 원인 제거] 여기서 save_state()를 제거하여 파일 교통사고 완벽 차단!
...
def install():
    ...
    for path, content in files.items():
        ...
        print(f"✅ V18 수술 및 마스터 패치 완료: {path}")
    print("\n🎉 노란 경고창도 지우고, 무한 로딩 교통사고도 완전히 고쳤습니다!")
    print("-----------------------------------------------------")
    print("1. 터미널 명령어: python main.py")
    print("2. [필수!] 접속 주소: http://127.0.0.1:8000 (0.0.0.0은 절대 금지!)")
    print("3. [필수!] 크롬에서 단축키 [Ctrl + Shift + R] 을 눌러 강력 새로고침 하세요!")
    print("-----------------------------------------------------")
