...
print("🔥 [Project Phoenix V42] 6대잼 10단계 진화 칭호 시스템 이식 중...")
...
# 2. Trader (V42: 10단계 진화 칭호 판별 알고리즘 추가)
...
class PhoenixTrader:
    ...
    # [V42 핵심] 10단계 칭호 판별기
    def get_ai_rank(self, profit, win_rate):
        if profit >= 2000000 and win_rate >= 60.0: return "[Lv.10] 갓 오브 피닉스 👑"
        elif profit >= 1000000: return "[Lv.9] 퀀텀 마인드 🌠"
        elif profit >= 500000: return "[Lv.8] 초월적 오라클 🌌"
        elif profit >= 200000: return "[Lv.7] 뉴럴 네트워크 🔮"
        elif profit >= 100000: return "[Lv.6] 딥러닝 프라임 💠"
        elif profit >= 50000: return "[Lv.5] 알고리즘 마스터 💎"
        elif profit >= 30000: return "[Lv.4] 퀀트 오퍼레이터 🎖️"
        elif profit >= 15000: return "[Lv.3] 로직 애널리스트 🏅"
        elif profit >= 5000: return "[Lv.2] 패턴 스캐너 🥈"
    ...
    async def get_portfolio_status(self):
        ...
        for name, data in self.agents.items():
            ...
            # [V42] 칭호 시스템 적용 (수익금 기준)
            rank_title = self.get_ai_rank(data["profit"], win_rate)
            ...
            ranking.append({
                ...
                "rank": rank_title  # 랭킹 정보에 칭호 추가
            })
            
            agent_details[name] = {
                "rank": rank_title, # 디테일 뷰에도 칭호 추가
...
# HTML UI (V42: UI 전역에 AI 진화 칭호(Rank) 배지 적용)
html_ui = """<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
    <title>PHOENIX AI TRADING V42</title>
    ...
    <header class="py-3 lg:h-14 lg:py-0 bg-panel border-b border-line flex flex-col lg:flex-row justify-between items-center px-4 shrink-0 gap-3">
        <div class="flex flex-col lg:flex-row items-center w-full lg:w-auto gap-3 overflow-hidden">
            <div class="flex items-center cursor-pointer shrink-0" onclick="setTab('trd')">
                ...
                <span class="text-[10px] text-yellow-500 border border-yellow-500/50 bg-yellow-900/30 px-1.5 py-0.5 rounded ml-2 font-bold tracking-wide">V42 EVO</span>
            </div>
            <nav class="flex space-x-5 text-sm font-bold ml-0 lg:ml-6 mt-2 lg:mt-0 w-full lg:w-auto overflow-x-auto whitespace-nowrap border-t lg:border-0 border-gray-800 pt-3 lg:pt-0 scrollbar-hide justify-center lg:justify-start">
                ...
                <a id="n-detail" onclick="setTab('detail')" class="text-gray-500 hover:text-white pb-2 lg:pb-[18px] lg:pt-5 border-b-2 border-transparent cursor-pointer px-1 flex items-center gap-1">📈 AI 진화상세</a>
        ...
        <div class="flex gap-4 items-center text-xs lg:text-sm text-gray-400 w-full justify-between lg:justify-end lg:w-auto mt-2 lg:mt-0 bg-[#0b1016] lg:bg-transparent p-2 lg:p-0 rounded border border-gray-800 lg:border-0 shrink-0">
            <div class="flex items-center gap-2">
                ...
                <span class="text-yellow-500 font-bold hidden md:inline">🤖 총 수익(금고포함):</span> 
                ...
                try {
                    ...
                    if(r.ranking) {
                        ...
                        r.ranking.forEach((bot, idx) => {
                            ...
                            vaultAgentsHtml += `<div class="bg-[#1b2029] border border-gray-700 rounded-xl p-5 flex flex-col justify-center shadow-lg"><div class="flex justify-between items-start mb-2"><div class="flex flex-col"><span class="text-white font-bold text-lg">${bot.name}</span><span class="text-[10px] font-bold text-yellow-500 mt-1">${bot.rank}</span></div><span class="text-xs px-2 py-1 bg-gray-800 text-gray-400 rounded-full">${idx+1}위</span></div><span class="text-2xl font-black text-yellow-500 num-font mt-2">+${vAmt.toLocaleString()} KRW</span><span class="text-[10px] text-gray-500 mt-1">이 AI가 벌어서 안전하게 빼낸 수익</span></div>`;
                        });
...
print("✅ V43 10단계 AI 진화 칭호(RPG 레벨업) 시스템 탑재 완료!")