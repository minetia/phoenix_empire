...
print("🔥 [Project Phoenix V16] 모든 기능이 통합된 마스터피스 설치 중...")
...
# 5. 트레이더 로직 (영구 DB 탑재 및 평단가/수익률 리얼 데이터 계산)
...
class PhoenixTrader:
    def __init__(self):
        ...
        self.ai_seed = 100000000.0 # 1억 원 원금
        ...
        if os.path.exists(self.state_file):
            try:
                with open(self.state_file, 'r', encoding='utf-8') as f:
                    ...
                    self.ai_krw = data.get("ai_krw", 100000000.0)
    ...
    async def simulate_ai_trading(self):
        while True:
            ...
            if side == "매수":
                ...
                old_qty = self.ai_hold.get(c, 0.0)
                old_avg = self.ai_avg.get(c, 0.0)
                new_qty = old_qty + qty
                new_avg = ((old_qty * old_avg) + bet) / new_qty if new_qty > 0 else 0
                self.ai_hold[c] = new_qty
                self.ai_avg[c] = new_avg
            else:
                hq = self.ai_hold.get(c, 0.0)
    ...
    async def get_portfolio_status(self):
        ...
        for c in self.port:
            ...
            ai_qty = self.ai_hold.get(c["code"], 0.0)
            ai_avg_price = self.ai_avg.get(c["code"], 0.0)
            ...
            ai_coin_pnl.append({
                ...
                "qty": float(ai_qty), "avg": float(ai_avg_price),
                "invested": float(ai_invested), "valuation": float(ai_valuation),
                "profit": float(ai_profit), "rate": float(ai_coin_rate)
            })
        ...
        return {
            ...
            "ai_krw": float(self.ai_krw), "ai_tot": float(tot_ai), 
            "ai_prof": float(ai_prof), "ai_rate": float(ai_rate),
    ...
    <header class="h-12 bg-panel border-b border-line flex justify-between items-center px-4 shrink-0">
        ...
        <div class="flex gap-5 items-center text-xs text-gray-400">
            <div class="flex items-center gap-2 bg-[#0b1016] px-3 py-1 rounded border border-gray-800">
                <span class="text-yellow-500 font-bold">🤖 AI 총 평가손익:</span> 
    ...
    <script>
        ...
        ws.onmessage = e => { 
            try {
                ...
                if(r.ai_tot) {
                    document.getElementById('ai-tot').innerText=Math.round(Number(r.ai_tot)).toLocaleString()+" KRW"; 
                    ...
                    document.getElementById('ai-total-val').innerText = Math.round(Number(r.ai_tot)).toLocaleString()+" KRW";
                    document.getElementById('ai-cash-val').innerText = Math.round(Number(r.ai_krw)).toLocaleString()+" KRW";
                    document.getElementById('ai-coin-val').innerText = Math.round(Number(r.ai_tot) - Number(r.ai_krw)).toLocaleString()+" KRW";
                }
                ...
                if(r.ai_coin_pnl) {
                    ...
                    document.getElementById('ai-coin-pnl-list').innerHTML = aiPnlHtml;
                }
...
def install():
    ...
    for path, content in files.items():
        ...
        print(f"✅ V16 궁극의 통합본 생성 완료: {path}")
    print("\n🎉 모든 설치가 끝났습니다! 터미널에서 python main.py 를 실행하세요.")
    print("👉 접속 주소: http://127.0.0.1:8000 (0.0.0.0 사용 금지!)")
    print("👉 접속 후 꼭 [Ctrl + Shift + R] 을 눌러 새로고침 해주세요!")
