...
print("🔥 [Project Phoenix V34] 긴급 심폐소생 및 무결점 방어막 가동 중...")

# [원인 제거] 에러를 유발하는 구버전 세이브 파일을 강제 소각합니다.
state_file = os.path.join(TARGET_PATH, "ai_state.json")
if os.path.exists(state_file):
    try: 
        os.remove(state_file)
        print("✅ 구버전 세이브 파일 강제 소각 완료!")
    except: pass

files = {}

# 1. Main Server (에러 로깅 추가)
...
# 2. Trader (V34: 완벽한 하위 호환성 검사 및 강제 리셋 로직 탑재)
...
class PhoenixTrader:
    ...
    def _generate_mock_history(self):
        ...
        for a_name, a_data in self.agents.items():
            if len(a_data.get("history", [])) == 0:
                base_asset = self.ai_seed
                cum_prof = 0
                for i in range(7, 0, -1):
                    d_date = (now - timedelta(days=i)).strftime("%m.%d")
                    d_prof = random.uniform(-500000, 500000)
                    cum_prof += d_prof
                    d_rate = (d_prof / base_asset) * 100
                    cum_rate = (cum_prof / self.ai_seed) * 100
                    end_asset = base_asset + d_prof
                    a_data.setdefault("history", []).append({
                        "date": d_date, "d_prof": d_prof, "c_prof": cum_prof,
                        "d_rate": d_rate, "c_rate": cum_rate,
                        "b_asset": base_asset, "e_asset": end_asset
                    })
        ...
        if os.path.exists(self.state_file):
            try:
                with open(self.state_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    ag = data.get("agents", {})
                    # [V34 방어막] V32 구버전 데이터가 들어오면 즉시 에러를 발생시켜 리셋시킴
                    if "전략 잼" in ag and "history" not in ag["전략 잼"]:
                        raise Exception("Corrupted old DB")
                    ...
                    self.agents = ag if ag else self.agents
    ...
    async def get_portfolio_status(self):
        ...
        for name, data in self.agents.items():
            ...
            avg_inv = self.ai_seed + (tot_p / 2) 
            
            agent_details[name] = {
                "summary": { "cum_prof": tot_p, "cum_rate": tot_r, "avg_inv": avg_inv },
...
# HTML UI (V34: 자바스크립트 철벽 방어 및 UI 렌더링 무결점 패치)
html_ui = """<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
    <title>PHOENIX AI TRADING V34</title>
    ...
    <style>
        ...
        .ub-sub { font-size: 11px; margin-top: 4px; }
...
<body class="md:h-screen flex flex-col select-none overflow-y-auto md:overflow-hidden bg-[#0b1016]">
    <header class="py-3 md:h-14 md:py-0 bg-panel border-b border-line flex flex-col md:flex-row justify-between items-center px-4 shrink-0 gap-3">
        <div class="flex flex-col md:flex-row items-center w-full md:w-auto gap-3 overflow-hidden">
            <div class="flex items-center cursor-pointer shrink-0" onclick="setTab('trd')">
                ...
                <span class="text-[10px] text-yellow-500 border border-yellow-500/50 bg-yellow-900/30 px-1.5 py-0.5 rounded ml-2 font-bold tracking-wide">V34</span>
            </div>
    ...
    <main id="v-trd" class="flex-1 flex flex-col md:flex-row overflow-y-auto md:overflow-hidden p-1 gap-1">
        <div class="flex flex-col space-y-1 w-full md:flex-1 md:min-w-[600px] h-auto md:h-full shrink-0">
            ...
            <div class="h-auto md:flex-[2] bg-panel flex flex-col md:flex-row shrink-0">
                <div class="w-full md:w-1/2 border-b md:border-b-0 md:border-r border-line flex flex-col h-[200px] md:h-auto"><div class="text-center text-xs py-1 border-b border-line text-gray-400 font-bold bg-[#0b1016]">일반호가</div><div id="ob" class="flex-1 p-1 text-[11px] md:text-xs num-font overflow-y-auto"></div></div>
                <div class="w-full md:w-1/2 flex flex-col bg-[#0b1016] h-auto md:h-auto"><div class="flex flex-col border-b border-line bg-panel p-2 gap-2"><div class="flex justify-between items-center"><span class="text-xs font-bold text-yellow-500">🧠 AI 설정</span><select onchange="changeMode(this.value)" class="bg-[#1b2029] text-xs text-white border border-gray-700 rounded p-1 outline-none"><option value="safe">🛡️ 안정형</option><option value="balance" selected>⚖️ 밸런스</option><option value="aggressive">⚡ 공격형</option></select></div><div class="flex justify-between text-[11px] bg-[#0b1016] p-1.5 rounded border border-gray-800"><span class="text-gray-400">AI 전체 시드:</span><span id="ai-tot" class="font-bold text-green-400">100,000,000 KRW</span></div></div><div id="ai-analysis-box" class="flex-1 p-3 space-y-2 text-xs overflow-y-auto border-t border-gray-800 min-h-[120px]"></div></div>
            </div>
        </div>
    ...
    <main id="v-detail" class="flex-1 hidden bg-[#0b1016] p-2 md:p-4 overflow-y-auto w-full">
        <div class="max-w-[800px] mx-auto mt-2 md:mt-4">
            ...
            <div class="bg-panel rounded-lg border border-gray-800 p-5 mb-8 shadow-xl">
                <div class="mb-8">
                    <div class="text-sm text-gray-400 flex items-center gap-1 mb-1"><span id="dt-period">기간 불러오는 중...</span> </div>
                    <div class="mb-4 text-sm text-gray-400 mt-6">기간 누적 손익</div>
                    <div id="dt-cum-prof" class="text-[32px] font-bold num-font mb-6 leading-none">0 KRW</div>
                    <div class="flex pt-6 pb-2 relative"><div class="absolute top-0 left-0 w-full h-[1px] bg-[#1f2937]"></div><div class="flex-1"><div class="text-[13px] text-gray-400 mb-1">기간 누적 수익률</div><div id="dt-cum-rate" class="text-lg num-font font-medium">0.00 %</div></div><div class="w-[1px] bg-[#1f2937] mx-4"></div><div class="flex-1"><div class="text-[13px] text-gray-400 mb-1">기간 평균 투자금액</div><div id="dt-avg-inv" class="text-lg text-gray-200 num-font font-medium">0 KRW</div></div></div>
                </div>
                <div class="border-t border-[#1f2937] pt-6 mb-8">
                    <div class="flex justify-between items-center mb-4"><h3 class="text-[15px] font-bold text-gray-200">투자손익 그래프</h3></div>
                    <div class="h-[200px] w-full relative"><canvas id="profitChart"></canvas></div>
                </div>
                <table class="w-full ub-table">
                    <thead><tr><th class="w-[20%] text-left pl-2">일자</th><th class="w-[30%]"><div class="mb-1">일일 손익</div><div>누적 손익</div></th><th class="w-[20%]"><div class="mb-1">일일 수익률</div><div>누적 수익률</div></th><th class="w-[30%] text-right pr-2"><div class="mb-1">기말 자산</div><div>기초 자산</div></th></tr></thead>
    ...
    <script>
        ...
        let globalAgentData = {}; let currentAgent = '전략 잼'; let chartInstance = null;

        // [V34 방어막] 차트 스크립트 로드 실패 시 무결점 렌더링 유지
        function loadChart() {
            try {
                ...
                if(typeof TradingView !== 'undefined') {
                    ...
                    document.getElementById('tv_chart_container').innerHTML = '<div class="flex items-center justify-center h-full text-gray-500">차트 플러그인 대기 중... 새로고침 해주세요.</div>';
                }
            } catch(e) {}
        }
        loadChart(); 
        ...
        function drawH() { 
            try {
                ...
                f.forEach(x=>{ 
                    ...
                    // [V34 방어막] 과거 찌꺼기 데이터에서 AI 이름이 없을 경우를 완벽 대비
                    const aiName = x.ai || "AI"; 
                    const shortAiName = aiName.split(' ')[0];
        ...
        function renderAnalysis() {
            ...
            document.getElementById('ai-analysis-box').innerHTML = `<div class="bg-[#1b2029] p-3 rounded border border-gray-700 space-y-2 text-xs md:text-sm"><div class="text-yellow-400 font-bold border-b border-gray-700 pb-1 mb-2">[${currentCoin}] 실시간 분석 브리핑</div><div class="flex justify-between"><span class="text-gray-400">시장 상태:</span><span class="font-bold text-white">${data.status}</span></div><div class="flex justify-between"><span class="text-gray-400">시뮬레이션 RSI:</span><span class="font-bold text-blue-400">${data.rsi}</span></div><div class="flex justify-between"><span class="text-gray-400">추천 6대잼:</span><span class="font-bold text-green-400">${data.rec}</span></div></div>`;
        }
        ...
        function renderDetailView() {
            try {
                ...
                if(data.history && data.history.length > 0) {
                    const safeHistory = [...data.history]; // V34 배열 복사 방어막
                    const dates = safeHistory.reverse().map(x => x.date);
                    const sDate = dates[0]; const eDate = dates[dates.length-1];
                    document.getElementById('dt-period').innerText = `2026년 ${sDate.replace('.', '월 ')}일 ~ ${eDate.replace('.', '월 ')}일의 투자손익`;
                    ...
                    if(typeof Chart !== 'undefined') {
                        ...
                        chartInstance = new Chart(ctx, {
                            ...
                            data: {
                                labels: dates,
                                datasets: [{
                                    data: safeHistory.map(x => x.c_rate),
                                    borderColor: lineColor, borderWidth: 2, pointRadius: 0, pointHoverRadius: 4, tension: 0.1
                                }]
                            },
                            options: {
                                responsive: true, maintainAspectRatio: false,
                                plugins: { legend: { display: false }, tooltip: { mode: 'index', intersect: false } },
                                scales: { x: { display: false }, y: { position: 'left', grid: { color: '#1f2937', drawBorder: false }, ticks: { color: '#6b7280', font: {size: 10} } } },
...
print("✅ V34 심폐소생술 완료! 이제 블랙아웃 현상은 절대 없습니다!")