...
print("🔥 [Project Phoenix V9] 일일 투자손익 상세 모듈 탑재 중...")
...
if __name__ == "__main__":
    import uvicorn
    # 반드시 127.0.0.1 로 실행 (지갑 피싱 경고 회피)
...
class AITradeLogger:
    ...
    def log_trade(self, ai, coin, side, qty, price):
        ...
        return {"time": t[11:], "full_time": t, "order_time": order_t, "ai": ai, "coin": coin, "market": "KRW", "side": side, "qty": qty, "price": price, "tot": tot, "fee": fee, "settle": settle}
"""
...
class PhoenixTrader:
    ...
    async def get_portfolio_status(self):
        ...
        usdt = prc.get("KRW-USDT", 1450) or 1450
        ...
        for c in self.port:
            ...
            status = "초강세 (과매수)" if rate > 5 else "상승 추세 (홀딩)" if rate > 0 else "조정 (분할매수)" if rate > -5 else "하락 추세 (관망)"
            analysis_data[sym] = {"status": status, "rec": "추세추종 잼" if rate > 0 else "방패 잼", "rsi": round(random.uniform(20, 80), 1)}
            
            res.append({"name": c["name"], "code": sym, "qty": c["qty"], "avg": c["avg"], "cur_krw": cp, "cur_usd": cp/usdt, "val": val, "prof": prof, "rate": rate})
        
        tot_ai = self.ai_krw + sum(qty * prc.get(code,0) for code, qty in self.ai_hold.items() if qty>0)
        
        # [V9 추가] 일일 투자손익 시뮬레이션 데이터 생성 (실시간 + 이미지 데이터 결합)
        today_str = datetime.now().strftime("%m.%d")
        
        # 실시간 변동 적용 (오늘 발생한 손익을 전체 손익의 일정 비율로 시뮬레이션)
        today_prof = total_prof * 0.05 + random.uniform(-50000, 50000)
        today_rate = (today_prof / total_invest) * 100 if total_invest > 0 else 0
        cum_rate = (total_prof / total_invest) * 100 if total_invest > 0 else 0

        daily_hist = [
            {"date": f"{today_str} (Live)", "d_prof": today_prof, "d_rate": today_rate, "e_asset": total_val, "c_prof": total_prof, "c_rate": cum_rate, "s_asset": total_val - today_prof},
            {"date": "02.18", "d_prof": 142015, "d_rate": 0.87, "e_asset": 16434265, "c_prof": -1574387, "c_rate": -10.08, "s_asset": 16292250},
            {"date": "02.17", "d_prof": -161321, "d_rate": -0.98, "e_asset": 16292250, "c_prof": -1716402, "c_rate": -11.01, "s_asset": 16453571},
            {"date": "02.16", "d_prof": -113039, "d_rate": -0.68, "e_asset": 16453571, "c_prof": -1555081, "c_rate": -10.01, "s_asset": 16566611},
            {"date": "02.15", "d_prof": -296894, "d_rate": -1.76, "e_asset": 16566611, "c_prof": -1442041, "c_rate": -9.33, "s_asset": 16863505},
            {"date": "02.14", "d_prof": 752809, "d_rate": 4.67, "e_asset": 16863505, "c_prof": -1145147, "c_rate": -7.46, "s_asset": 16110695},
        ]
        
        return {"usdt": usdt, "data": res, "hist": self.hist, "ai_tot": tot_ai, "analysis": analysis_data, "daily": daily_hist}
"""

# ==================================
# 4. HTML / UI (V9: 일일손익 상세 뷰 추가)
# ==================================
html_part1 = """<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8"><title>Phoenix PRO - AI Trading V9</title>
...
<body class="h-screen flex flex-col select-none">
    <header class="h-12 bg-panel border-b border-line flex justify-between items-center px-4 shrink-0">
        <div class="flex items-center space-x-6">
            <h1 class="text-lg font-bold text-white tracking-wider">UPBIT <span class="text-yellow-500 text-sm border border-yellow-500 px-1 rounded ml-1">AI PRO V9</span></h1>
...
html_part2 = """
    <main id="v-trd" class="flex-1 flex overflow-hidden p-1 space-x-1">
        <div class="flex-1 flex flex-col space-y-1 overflow-hidden min-w-[600px]">
            ...
            <div class="flex-[3] bg-panel" id="tv_chart_container">
            ...
            <div class="flex-[2] bg-panel flex">
                ...
                <div class="w-1/2 flex flex-col bg-[#0b1016]">
                    ...
                    <div id="ai-analysis-box" class="flex-1 p-3 space-y-2 text-[11px] overflow-y-auto border-t border-gray-800"></div>
                </div>
            </div>
        </div>
        ...
        <div class="w-[380px] flex flex-col bg-panel shrink-0">
            <div class="p-2 border-b border-line text-xs text-yellow-500 font-bold bg-[#1b2029]">💡 클릭 시 차트/분석 변경</div>
...
html_part3 = """
    <main id="v-port" class="flex-1 hidden bg-[#0b1016] p-4 overflow-y-auto w-full">
        <div class="max-w-[1200px] mx-auto mt-4 pb-20">
            ...
            <div class="bg-panel rounded-lg p-6 mb-6 border border-gray-800 shadow-lg flex justify-between">
                ...
                <div class="text-right"><p class="text-sm text-gray-400 mb-1">총 평가손익</p><p id="p-tot-prof" class="text-3xl font-bold num-font">0 KRW</p></div>
            </div>
            ...
            <h3 class="text-xl font-bold mb-4 text-white flex items-center gap-2">📊 일일 투자손익 상세 <span class="text-xs text-gray-500 font-normal ml-2">KRW 환산 추정값</span></h3>
            <div class="bg-panel rounded-lg border border-gray-800 overflow-hidden w-full mb-8">
                <div class="flex text-[11px] text-gray-400 py-3 px-4 border-b border-gray-800 bg-[#0b1016]">
                    <div class="w-[15%] flex items-center">일자</div>
                    <div class="w-[30%] text-right"><div class="mb-1">일일 손익</div><div>누적 손익</div></div>
                    <div class="w-[25%] text-right"><div class="mb-1">일일 수익률</div><div>누적 수익률</div></div>
                    <div class="w-[30%] text-right"><div class="mb-1 text-gray-300">기말 자산</div><div>기초 자산</div></div>
                </div>
                <div id="daily-pnl-list" class="divide-y divide-gray-800 text-sm">
                    <div class="p-4 text-center text-gray-500">데이터 로딩 중...</div>
                </div>
            </div>

            <h3 class="text-xl font-bold mb-4 text-yellow-500 flex items-center gap-2">🤖 AI 상세 거래내역 (장부)</h3>
            <div class="bg-panel rounded-lg border border-gray-800 overflow-hidden w-full overflow-x-auto">
                <div class="flex text-[11px] text-gray-400 py-2 px-4 border-b border-gray-800 bg-[#0b1016] min-w-[1000px]">
                    <div class="w-[12%]">주문시간</div><div class="w-[12%]">체결시간</div><div class="w-[10%]">코인명</div><div class="w-[8%] text-center">종류</div><div class="w-[10%] text-right">수량</div><div class="w-[12%] text-right">단가</div><div class="w-[12%] text-right">금액</div><div class="w-[10%] text-right">수수료</div><div class="w-[14%] text-right">정산금액</div>
                </div>
                <div id="ai-detail-list" class="divide-y divide-gray-800 min-w-[1000px]"></div>
            </div>
        </div>
    </main>
    ...
    <script>
        ...
        function drawH() { 
            ...
            gh.forEach(x => {
                const c=x.side==='매수'?'up-red':'down-blue'; 
                adH += `<div class="flex py-2 px-4 hover:bg-gray-800 transition text-[11px] min-w-[1000px] items-center border-b border-gray-800"><div class="w-[12%] text-gray-500">${x.order_time}</div><div class="w-[12%] text-gray-400 font-bold">${x.full_time}</div><div class="w-[10%] font-bold text-gray-200">${x.coin}</div><div class="w-[8%] font-bold text-center ${c} bg-opacity-20 rounded p-1 ${x.side==='매수'?'bg-red-900':'bg-blue-900'}">${x.side}</div><div class="w-[10%] text-right num-font">${x.qty.toFixed(4)}</div><div class="w-[12%] text-right num-font">${x.price.toLocaleString()}</div><div class="w-[12%] text-right num-font">${x.tot.toLocaleString()}</div><div class="w-[10%] text-right num-font text-gray-500">${x.fee.toLocaleString()}</div><div class="w-[14%] text-right font-bold num-font ${c}">${x.settle.toLocaleString()}</div></div>`;
            });
        ...
        function renderAnalysis() {
            ...
            document.getElementById('ai-analysis-box').innerHTML = `
                <div class="bg-[#1b2029] p-3 rounded border border-gray-700 space-y-2">
                    <div class="text-yellow-400 font-bold border-b border-gray-700 pb-1 mb-2">[${currentCoin}] 분석 브리핑</div>
                    <div class="flex justify-between"><span class="text-gray-400">시장 상태:</span><span class="font-bold text-white">${data.status}</span></div>
                    <div class="flex justify-between"><span class="text-gray-400">RSI 시그널:</span><span class="font-bold text-blue-400">${data.rsi}</span></div>
                    <div class="flex justify-between"><span class="text-gray-400">추천 잼:</span><span class="font-bold text-green-400">${data.rec}</span></div>
                </div>`;
        }
        ...
        ws.onmessage = e => { 
            try {
                ...
                if(r.ai_tot) document.getElementById('ai-tot').innerText=Math.round(Number(r.ai_tot)).toLocaleString()+" KRW"; 
                ...
                // [V9 신규] 일일 투자손익 상세 렌더링
                if(r.daily) {
                    let dHtml = "";
                    r.daily.forEach(d => {
                        const dp_cls = d.d_prof >= 0 ? 'up-red' : 'down-blue';
                        const cp_cls = d.c_prof >= 0 ? 'up-red' : 'down-blue';
                        dHtml += `
                        <div class="flex py-3 px-4 hover:bg-[#1b2029] transition border-b border-gray-800 items-center">
                            <div class="w-[15%] text-gray-300 font-bold">${d.date}</div>
                            <div class="w-[30%] text-right flex flex-col gap-1">
                                <span class="${dp_cls} num-font">${Math.round(d.d_prof).toLocaleString()}</span>
                                <span class="${cp_cls} num-font">${Math.round(d.c_prof).toLocaleString()}</span>
                            </div>
                            <div class="w-[25%] text-right flex flex-col gap-1">
                                <span class="${dp_cls} num-font">${d.d_rate.toFixed(2)}%</span>
                                <span class="${cp_cls} num-font">${d.c_rate.toFixed(2)}%</span>
                            </div>
                            <div class="w-[30%] text-right flex flex-col gap-1">
                                <span class="text-gray-200 num-font">${Math.round(d.e_asset).toLocaleString()}</span>
                                <span class="text-gray-400 num-font">${Math.round(d.s_asset).toLocaleString()}</span>
                            </div>
                        </div>`;
                    });
                    document.getElementById('daily-pnl-list').innerHTML = dHtml;
                }
...
def install():
    ...
    for path, content in files.items():
        ...
        print(f"✅ V9 파일 덮어쓰기 완료: {path}")
    print("\n🎉 V9: 일일 투자손익 상세 화면이 완벽하게 설치되었습니다!")
