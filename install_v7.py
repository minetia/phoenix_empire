...
print("🔥 [Project Phoenix V7] 차트 연동 및 상세 AI 투자내역 패치 중...")
...
class PhoenixTrader:
    ...
    async def get_portfolio_status(self):
        ...
        for c in self.port:
            ...
            analysis_data[sym] = {
                "status": status, "rec": rec_jam, "rsi": rsi_sim, 
                "vol": f"{random.randint(100, 999)}K"
            }
            
            res.append({
                "name": c["name"], "code": sym, "qty": c["qty"], "avg": c["avg"], 
        ...
        return {"usdt": usdt, "data": res, "hist": self.hist, "ai_tot": tot_ai, "analysis": analysis_data}
"""

html_part1 = """<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8"><title>Phoenix PRO - AI Trading V7</title>
...
<body class="h-screen flex flex-col select-none">
    <header class="h-12 bg-panel border-b border-line flex justify-between items-center px-4 shrink-0">
        <div class="flex items-center space-x-6">
            <h1 class="text-lg font-bold text-white tracking-wider">UPBIT <span class="text-yellow-500 text-sm border border-yellow-500 px-1 rounded ml-1">AI PRO V7</span></h1>
...
html_part2 = """
    <main id="v-trd" class="flex-1 flex overflow-hidden p-1 space-x-1">
        <div class="flex-1 flex flex-col space-y-1 overflow-hidden min-w-[600px]">
            ...
            <div class="flex-[2] bg-panel flex">
                ...
                <div class="w-1/2 flex flex-col bg-[#0b1016]">
                    ...
                    <div id="ai-analysis-box" class="flex-1 p-3 space-y-2 text-[11px] overflow-y-auto border-t border-gray-800">
                        <div class="text-center text-gray-500 mt-4 animate-pulse">AI 실시간 분석 중...</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="w-[380px] flex flex-col bg-panel shrink-0">
            <div class="p-2 border-b border-line text-xs text-yellow-500 font-bold bg-[#1b2029]">💡 코인을 클릭하면 차트와 분석이 연동됩니다.</div>
...
html_part3 = """
    <main id="v-port" class="flex-1 hidden bg-[#0b1016] p-4 overflow-y-auto w-full">
        <div class="max-w-[1200px] mx-auto mt-4">
            ...
            <h3 class="text-xl font-bold mb-4 text-yellow-500 flex items-center gap-2">🤖 AI 상세 거래내역 (장부) <span class="text-xs text-gray-500 font-normal ml-2">ai_trade_log.csv 동기화 중</span></h3>
            <div class="bg-panel rounded-lg border border-gray-800 overflow-hidden w-full overflow-x-auto">
                <div class="flex text-[11px] text-gray-400 py-2 px-4 border-b border-gray-800 bg-[#0b1016] min-w-[1000px]">
                    <div class="w-[12%]">주문시간</div><div class="w-[12%]">체결시간</div><div class="w-[10%]">코인명(마켓)</div><div class="w-[8%] text-center">종류</div><div class="w-[10%] text-right">거래수량</div><div class="w-[12%] text-right">거래단가</div><div class="w-[12%] text-right">거래금액</div><div class="w-[10%] text-right">수수료</div><div class="w-[14%] text-right">정산금액(수수료반영)</div>
                </div>
    ...
    <script>
        ...
        function selectCoin(code, name) {
            ...
            document.getElementById('m-code').innerText = code + "/KRW";
            loadChart();
            renderAnalysis();
        }
        ...
        async function changeMode(m) { 
            ...
            currentMode = m === 'safe' ? '안정형' : m === 'balance' ? '밸런스' : '공격형';
            renderAnalysis();
        }
        ...
        function drawH() { 
            ...
            // 포트폴리오 뷰의 상세 리스트 렌더링 (10개 항목)
            gh.forEach(x => {
                const c=x.side==='매수'?'up-red':'down-blue'; 
                adH += `<div class="flex py-2 px-4 hover:bg-gray-800 transition text-[11px] min-w-[1000px] items-center border-b border-gray-800">
                    <div class="w-[12%] text-gray-500">${x.order_time}</div>
                    <div class="w-[12%] text-gray-400 font-bold">${x.full_time}</div>
                    <div class="w-[10%] font-bold text-gray-200">${x.coin} (KRW)</div>
                    <div class="w-[8%] font-bold text-center ${c} bg-opacity-20 rounded p-1 ${x.side==='매수'?'bg-red-900':'bg-blue-900'}">${x.side}</div>
                    <div class="w-[10%] text-right num-font">${x.qty.toFixed(4)}</div>
                    <div class="w-[12%] text-right num-font">${x.price.toLocaleString()}</div>
                    <div class="w-[12%] text-right num-font">${x.tot.toLocaleString()}</div>
                    <div class="w-[10%] text-right num-font text-gray-500">${x.fee.toLocaleString()}</div>
                    <div class="w-[14%] text-right font-bold num-font ${c}">${x.settle.toLocaleString()}</div>
                </div>`;
            });

            document.getElementById('h-list').innerHTML=h; 
            document.getElementById('ai-detail-list').innerHTML=adH || `<div class="p-4 text-center text-gray-500">AI 거래 내역이 아직 없습니다.</div>`;
        }
        ...
        function renderAnalysis() {
            ...
            const html = `
                <div class="bg-[#1b2029] p-3 rounded border border-gray-700 space-y-2">
                    ...
                    <div class="mt-2 text-gray-500 pt-2 border-t border-gray-800">※ 설정된 [${currentMode}] 모드에 따라 5대잼이 알고리즘을 최적화하고 있습니다.</div>
                </div>`;
            document.getElementById('ai-analysis-box').innerHTML = html;
        }
        ...
        ws.onmessage = e => { 
            ...
            r.data.forEach(c => { 
                ...
                // 현재 클릭된 코인일 경우 좌측 상단 패널 및 호가창 업데이트
                ...
                // 우측 거래소 코인 리스트 (클릭 이벤트 추가됨)
                lh+=`<div onclick="selectCoin('${c.code}', '${c.name}')" class="flex text-[11px] py-1.5 px-2 hover:bg-[#1b2029] items-center cursor-pointer transition">
...
def install():
    ...
    for path, content in files.items():
        ...
        print(f"✅ V7 파일 생성 성공: {path}")
    print("\n🎉 모든 고급 기능이 완벽하게 패치되었습니다!")
