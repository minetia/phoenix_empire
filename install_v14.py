...
print("🔥 [Project Phoenix V14] 영구 기억 장치(DB) 탑재 중...")
...
class PhoenixTrader:
    def __init__(self):
        ...
        self.ai_seed = 100000000.0 # 1억 원 (초기 자본, 고정)
    ...
    def load_state(self):
        if os.path.exists(self.state_file):
            try:
                ...
                print("State load error, resetting:", e)
    ...
    async def get_portfolio_status(self):
        ...
        daily_pnl = [{
            "date": today_str, 
            "d_prof": float(ai_prof), "a_prof": float(ai_prof),
            "d_rate": float(ai_rate), "a_rate": float(ai_rate),
            "s_asset": float(self.ai_seed), "e_asset": float(tot_ai)
        }]
...
html_part3 = """
    ...
    <main id="v-pnl" class="flex-1 hidden bg-[#0b1016] p-4 overflow-y-auto w-full">
        <div class="max-w-[1200px] mx-auto mt-4">
            <h2 class="text-2xl font-bold mb-6 text-white border-b border-gray-800 pb-2 flex items-center gap-2">📊 일일 투자손익 상세 <span class="text-sm font-normal text-gray-500">AI 트레이딩 실제 성과</span></h2>
            <div class="bg-panel rounded-lg border border-gray-800 overflow-hidden w-full">
                <div class="flex text-xs text-gray-400 py-4 px-6 border-b border-gray-800 bg-[#0b1016]">
                    ...
                    <div class="w-[30%] text-right">기말 자산<br>기초 자산 (초기 시드: 1억)</div>
                </div>
...
def install():
    ...
    for path, content in files.items():
        ...
        print(f"✅ V14 [영구 저장 메모리] 생성 완료: {path}")
    print("\n🎉 모든 설치가 끝났습니다! 이제 새로고침해도 데이터가 날아가지 않습니다.")
