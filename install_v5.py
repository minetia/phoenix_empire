...
print("🔥 [Project Phoenix V5.1] 투자내역 화면 복구 패치를 시작합니다...")
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    while True:
        await websocket.send_json(await trader.get_portfolio_status())
...
class PhoenixTrader:
    ...
    async def simulate_ai_trading(self):
        while True:
            ...
            p = self.ex.get_current_price([c]).get(c, 0)
            if p == 0: continue
...
<html lang="ko">
<head>
    <meta charset="UTF-8"><title>Phoenix PRO - AI Trading V5</title>
...
<body class="h-screen flex flex-col select-none">
    <header class="h-12 bg-panel border-b border-line flex justify-between items-center px-4 shrink-0">
        <div class="flex items-center space-x-6">
            <h1 class="text-lg font-bold text-white tracking-wider">UPBIT <span class="text-yellow-500 text-sm border border-yellow-500 px-1 rounded ml-1">AI PRO V5.1</span></h1>
    ...
    <main id="v-trd" class="flex-1 flex overflow-hidden p-1 space-x-1">
            ...
            <div class="flex-[2] bg-panel flex">
                ...
                <div class="w-1/2 flex flex-col bg-[#0b1016]">
                    <div class="flex flex-col border-b border-line bg-panel p-2 gap-2">
                        <div class="flex justify-between items-center"><span class="text-xs font-bold text-yellow-500">🧠 AI 트레이딩 설정</span>
        ...
        <div class="w-[350px] flex flex-col bg-panel shrink-0">
            <div class="flex text-sm border-b border-line bg-[#0b1016]">
                ...
                <div id="t-hist" class="flex-1 text-center py-2 o-hist" onclick="setOrd('history')">거래내역 (AI)</div>
            </div>
...
# HTML 파트 3: 복구된 투자내역 뷰 및 자바스크립트
html_part3 = """
    <main id="v-port" class="flex-1 hidden bg-[#0b1016] p-4 overflow-y-auto">
        <div class="max-w-4xl mx-auto mt-4">
            ...
            <div class="bg-panel rounded-lg border border-gray-800 overflow-hidden">
                <div class="flex text-xs text-gray-400 py-3 px-4 border-b border-gray-800 bg-[#0b1016]">
                    <div class="w-1/4">자산명</div><div class="w-1/4 text-right">보유수량</div><div class="w-1/4 text-right">매수평균가</div><div class="w-1/4 text-right">평가손익 / 수익률</div>
                </div>
    ...
    <script>
        ...
        function setTab(t) { 
            const nTrd = document.getElementById('n-trd'); const nPort = document.getElementById('n-port');
            document.getElementById('v-trd').className = t==='trd' ? "flex-1 flex overflow-hidden p-1 space-x-1" : "hidden"; 
            document.getElementById('v-port').className = t==='port' ? "flex-1 bg-[#0b1016] p-4 overflow-y-auto" : "hidden"; 
            if(t==='trd') { nTrd.className="text-white border-b-2 border-white pb-[14px] pt-4 cursor-pointer"; nPort.className="text-gray-500 hover:text-white pb-[14px] pt-4 border-b-2 border-transparent cursor-pointer"; }
            else { nPort.className="text-white border-b-2 border-white pb-[14px] pt-4 cursor-pointer"; nTrd.className="text-gray-500 hover:text-white pb-[14px] pt-4 border-b-2 border-transparent cursor-pointer"; }
        }
        ...
        function drawH() { 
            ...
            let h=""; 
            f.forEach(x=>{ const c=x.side==='매수'?'up-red':'down-blue'; h+=`<div class="flex py-1.5 px-2 hover:bg-gray-800"><div class="w-1/5 text-gray-500">${x.time}</div><div class="w-1/4 font-bold text-gray-300 truncate">${x.ai}</div><div class="w-1/5 ${c} font-bold text-[10px]">${x.coin} ${x.side}</div><div class="w-1/3 text-right font-bold ${c}">${x.price.toLocaleString()}</div></div>`}); 
        ...
        ws.onmessage = e => { 
            ...
            document.getElementById('usdt').innerText=Math.round(r.usdt).toLocaleString(); 
            if(r.ai_tot) document.getElementById('ai-tot').innerText=Math.round(r.ai_tot).toLocaleString()+" KRW"; 
            
            let lh="", ph="", tp=0, tv=0; 
            r.data.forEach(c => { 
                tp+=c.prof; tv+=c.val;
                const cl=c.rate>=0?'up-red':'down-blue', s=c.rate>=0?'+':''; 
                if(c.code==='BTC'){ 
                    document.getElementById('m-prc').innerText=Math.round(c.cur_krw).toLocaleString(); 
                    document.getElementById('m-rt').innerText=`${s}${c.rate.toFixed(2)}%`; 
                    ...
                    drawOB(Math.round(c.cur_krw)); 
                } 
                lh+=`<div class="flex text-[11px] py-1.5 px-2 hover:bg-[#1b2029] items-center"><div class="flex-[1.5] flex flex-col"><span class="font-bold text-gray-200">${c.name}</span><span class="text-[9px] text-gray-500">${c.code}/KRW</span></div><div class="flex-[1.5] text-right flex flex-col"><span class="text-gray-300 num-font">${c.qty.toLocaleString(undefined,{maximumFractionDigits:4})}</span><span class="text-gray-500 text-[9px] num-font">${Math.round(c.avg).toLocaleString()}</span></div><div class="flex-[1.5] text-right font-bold num-font ${cl}">${Math.round(c.cur_krw).toLocaleString()}</div><div class="flex-[1.5] text-right flex flex-col"><span class="${cl} font-bold num-font">${s}${c.rate.toFixed(2)}%</span><span class="${cl} text-[9px] num-font">${Math.round(c.prof).toLocaleString()}</span></div></div>`; 
                ph+=`<div class="flex text-sm py-4 px-4 items-center hover:bg-gray-800 transition"><div class="w-1/4 flex flex-col"><span class="font-bold text-gray-200 text-base">${c.name}</span><span class="text-xs text-gray-500">${c.code}</span></div><div class="w-1/4 text-right num-font text-gray-300">${c.qty.toLocaleString(undefined,{maximumFractionDigits:4})}</div><div class="w-1/4 text-right num-font text-gray-400">${Math.round(c.avg).toLocaleString()} KRW</div><div class="w-1/4 text-right flex flex-col"><span class="${cl} font-bold num-font text-base">${Math.round(c.prof).toLocaleString()} KRW</span><span class="${cl} text-xs num-font">${s}${c.rate.toFixed(2)}%</span></div></div>`;
            }); 
...
def install():
    ...
    for path, content in files.items():
        ...
        print(f"✅ 파일 재생성 완료: {path}")
    print("\n🎉 완벽하게 패치되었습니다! 이제 터미널에서 python main.py 를 실행하세요.")
