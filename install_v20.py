...
print("🔥 [Project Phoenix V20] 렉(Lag) 제로! 투트랙 초고속 엔진 장착 중...")

# [필수] 꼬여버린 과거 에러 데이터 완전 소각
state_file = os.path.join(TARGET_PATH, "ai_state.json")
if os.path.exists(state_file):
    try:
        os.remove(state_file)
        print("💉 바이러스에 감염된 세이브 파일을 완전히 소각했습니다.")
    except:
        pass
...
# 1. Main Server (V20: 수명주기를 이용한 투트랙 백그라운드 구동)
...
async def lifespan(app: FastAPI):
    # [V20 핵심] 시세 수집 봇과 트레이딩 봇을 서로 방해하지 않게 두 개의 트랙으로 분리!
...
class PhoenixTrader:
    def __init__(self):
        ...
        self.prc_cache = {} # 투트랙 엔진 시세 공유 메모리
    ...
    # [V20 핵심 트랙 2] 시세 대기 렉 없이 즉각 매매하는 트레이딩 봇
    ...
    async def get_portfolio_status(self):
        ...
        tot_ai = safe_num(self.ai_krw) + sum(safe_num(qty) * safe_num(self.prc_cache.get(code,0)) for code, qty in self.ai_hold.items() if safe_num(qty)>0)
...
# HTML (디자인 유지 + 자바스크립트 철벽 방어 코드 추가)
...
html_part3 = """
    ...
    <script>
        // [V20 핵심] 어떤 경우에도 멈추지 않는 철벽 방어 숫자 변환기
        const sn = (v, defVal=0) => {
            let n = Number(v);
            return (!isNaN(n) && isFinite(n)) ? n : defVal;
        };
        ...
        ws.onmessage = e => { 
            try {
                ...
                if(r.ai_tot !== undefined) {
                    const aiTot = sn(r.ai_tot);
                    const aiKrw = sn(r.ai_krw);
                    const aiProf = sn(r.ai_prof);
                    const aiRate = sn(r.ai_rate);
                    ...
                    if(elAiTotalVal) elAiTotalVal.innerText = Math.round(aiTot).toLocaleString()+" KRW";
                    const elAiCashVal = document.getElementById('ai-cash-val');
                    if(elAiCashVal) elAiCashVal.innerText = Math.round(aiKrw).toLocaleString()+" KRW";
                    const elAiCoinVal = document.getElementById('ai-coin-val');
                    if(elAiCoinVal) elAiCoinVal.innerText = Math.round(aiTot - aiKrw).toLocaleString()+" KRW";
                    const elAiTotNav = document.getElementById('ai-tot');
                    if(elAiTotNav) elAiTotNav.innerText = Math.round(aiTot).toLocaleString()+" KRW";
                }
                ...
                let lh="", ph="", tp=0, tv=0; 
                r.data.forEach(c => { 
                    const prof = sn(c.prof); const val = sn(c.val); const rate = sn(c.rate);
                    const cur_krw = sn(c.cur_krw); const avg = sn(c.avg); const qty = sn(c.qty);
                    tp += prof; tv += val;
                ...
                if(r.daily_pnl) {
                    ...
                    r.daily_pnl.forEach(d => {
                        const d_prof = sn(d.d_prof); const a_prof = sn(d.a_prof);
                        const d_rate = sn(d.d_rate); const a_rate = sn(d.a_rate);
...
def install():
    ...
    for path, content in files.items():
        ...
        print(f"✅ V20 초고속 투트랙 엔진 패치 완료: {path}")
    print("\n🎉 서버 렉(Lag)과 무한 로딩이 완전히 사라졌습니다!")
    print("-----------------------------------------------------")
    print("1. 터미널 명령어: python main.py")
    print("2. 크롬 접속 주소: http://127.0.0.1:8000")
    print("3. 접속 후 꼭 [Ctrl + Shift + R] 을 눌러 강력 새로고침 하세요!")
    print("-----------------------------------------------------")
