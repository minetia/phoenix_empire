...
print("🔥 [Project Phoenix V30] 트레이딩뷰 지표 연동 '6대잼' 각성 패치 중...")

# 뇌 구조(DB)가 완전히 바뀌었으므로 기존 세이브 파일 삭제 후 초기화
...
if __name__ == "__main__":
    ...
    print("\\n" + "★"*60)
    print("🦅 [Project Phoenix V30] 6대잼(6-Jams) 시스템이 가동되었습니다!")
    print("★"*60)
...
# 2. Trader (V30: 트레이딩뷰 지표 기반 6대잼 고유 알고리즘 및 승률 시스템 탑재)
...
class PhoenixTrader:
    def __init__(self):
        ...
        # [V30 핵심] 승률(wins/losses)이 추가된 6대잼 DB
        self.agents = {
            "전략 잼 (Strategist)": {"profit": 0.0, "holds": {c["code"]: 0.0 for c in self.port}, "wins": 0, "losses": 0},
            "수집 잼 (Collector)": {"profit": 0.0, "holds": {c["code"]: 0.0 for c in self.port}, "wins": 0, "losses": 0},
            "정렬 잼 (Sorter)": {"profit": 0.0, "holds": {c["code"]: 0.0 for c in self.port}, "wins": 0, "losses": 0},
            "타이밍 잼 (Sniper)": {"profit": 0.0, "holds": {c["code"]: 0.0 for c in self.port}, "wins": 0, "losses": 0},
            "가디언 잼 (Guardian)": {"profit": 0.0, "holds": {c["code"]: 0.0 for c in self.port}, "wins": 0, "losses": 0},
            "행동 잼 (Executor)": {"profit": 0.0, "holds": {c["code"]: 0.0 for c in self.port}, "wins": 0, "losses": 0}
        }
    ...
    async def simulate_ai_trading(self):
        while True:
            ...
            # [V30 핵심] 트레이딩뷰 지표 기반 6대잼 알고리즘
            if ai_name == "전략 잼 (Strategist)": # MACD 추세 추종
            ...
            elif ai_name == "수집 잼 (Collector)": # 눌림목 분할 매수 (DCA)
            ...
            elif ai_name == "정렬 잼 (Sorter)": # 포트폴리오 리밸런싱
                if bot_hold > 0 and profit_rate > 1.0: side = "매도"; sell_ratio = 0.5
                elif profit_rate < -1.0: side = "매수"; bet_ratio = 0.05
                else: continue
                
            elif ai_name == "타이밍 잼 (Sniper)": # RSI/볼린저 극단값 원샷원킬
                if profit_rate < -7.0 or random.random() < 0.02: side = "매수"; bet_ratio = 0.3 
                elif bot_hold > 0 and profit_rate > 6.0: side = "매도"; sell_ratio = 1.0
                else: continue
                
            elif ai_name == "가디언 잼 (Guardian)": # 리스크 관리 (칼손절, 익절)
                if bot_hold > 0 and profit_rate < -3.0: side = "매도"; sell_ratio = 1.0 # 손절
                elif bot_hold > 0 and profit_rate > 3.0: side = "매도"; sell_ratio = 1.0 # 익절
                elif profit_rate < -10.0: side = "매수"; bet_ratio = 0.2 # 극한의 패닉셀 줍기
                else: continue
                
            elif ai_name == "행동 잼 (Executor)": # 초고주파 스캘핑
            ...
            if side == "매수":
                ...
                # [V30 핵심] 승률 계산용 데이터 축적
                trade_profit = sell_amount - buy_amount
                self.agents[ai_name]["profit"] += trade_profit
                if trade_profit > 0:
                    self.agents[ai_name]["wins"] = self.agents[ai_name].get("wins", 0) + 1
                else:
                    self.agents[ai_name]["losses"] = self.agents[ai_name].get("losses", 0) + 1
...
# HTML UI (V30: 모바일 빈 공간 제거, 랭킹 리스트에 승률 추가, 차트 최적화)
    ...
    <header class="py-3 md:h-14 md:py-0 bg-panel border-b border-line flex flex-col md:flex-row justify-between items-center px-4 shrink-0 gap-3">
        <div class="flex flex-col md:flex-row items-center w-full md:w-auto gap-3 overflow-hidden">
            <div class="flex items-center cursor-pointer shrink-0" onclick="setTab('trd')">
                ...
                <span class="text-[10px] text-yellow-500 border border-yellow-500/50 bg-yellow-900/30 px-1.5 py-0.5 rounded ml-2 font-bold tracking-wide">AI CORE V30</span>
            </div>
...
print("✅ V30 6대잼 승률 시스템 및 차트 최적화 완벽 적용!")