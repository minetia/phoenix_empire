...
print("🔥 [Project Phoenix V40] 마스터 비밀 금고 생성 및 수익 자동 출금 시스템 장착 중...")
...
# 2. Trader (V40: 수익금 자동 계산 및 금고(Vault) 이체 로직, 풀악셀 매매 적용)
...
class PhoenixTrader:
    ...
    async def simulate_ai_trading(self):
        while True:
            ...
            # 🔥 [V40 핵심] 자동 수익금 출금 로직 (금고 이동)
            # 목표치: 20만원 / 6명 = 각 AI당 33,333.33원 수익 달성 시 출금
            ...
            if current_ai_asset >= self.ai_seed + 33333.33:
                ...
                # 현금(Cash)에서만 뺄 수 있으므로 현금 잔고 체크
                sweep_amt = min(excess, self.agents[ai_name]["cash"])
                
                if sweep_amt > 1000: # 너무 작은 금액은 안 뺌
                    self.agents[ai_name]["vault"] += sweep_amt
                    self.agents[ai_name]["cash"] -= sweep_amt
                    self.ai_krw -= sweep_amt # 전체 1억 시드풀에서도 차감하여 1억 유지!
                    
                    # 수익 출금 내역을 화면에 멋지게 띄움
                    now = datetime.now()
    ...
    async def get_portfolio_status(self):
        ...
        for data in self.agents.values():
            ...
            global_vault += data.get("vault", 0.0) # 총 금고 금액 합산
        ...
        tot_ai = safe_num(self.ai_krw) + tot_ai_coins # 1억 풀
        ai_prof = (tot_ai + global_vault) - 100000000.0 # 누적 손익 = (현재 1억풀 + 금고) - 초기 1억
        ...
        for name, data in self.agents.items():
            ...
            ranking.append({
                ...
                "vault": data.get("vault", 0.0) # 랭킹에도 금고 정보 전송
            })
        ...
        return {
            ...
            "global_vault": global_vault # 마스터 금고 총액 전송
        }
"""

# HTML UI (V40: 🔒 비밀 금고 PIN 시스템 및 완벽 방어 렌더링 코드 적용)
...
<head>
    ...
    <style>
        ...
        .o-vault { background:rgba(202,138,4,0.1); color:#eab308; border-top:2px solid #eab308; font-weight:bold; }
...
<body class="lg:h-screen flex flex-col select-none overflow-y-auto lg:overflow-hidden bg-[#0b1016]">
    ...
    <header class="py-3 lg:h-14 lg:py-0 bg-panel border-b border-line flex flex-col lg:flex-row justify-between items-center px-4 shrink-0 gap-3">
        <div class="flex flex-col lg:flex-row items-center w-full lg:w-auto gap-3 overflow-hidden">
            <div class="flex items-center cursor-pointer shrink-0" onclick="setTab('trd')">
                ...
                <span class="text-[10px] text-yellow-500 border border-yellow-500/50 bg-yellow-900/30 px-1.5 py-0.5 rounded ml-2 font-bold tracking-wide">V40</span>
            </div>
            <nav class="flex space-x-5 text-sm font-bold ml-0 lg:ml-6 mt-2 lg:mt-0 w-full lg:w-auto overflow-x-auto whitespace-nowrap border-t lg:border-0 border-gray-800 pt-3 lg:pt-0 scrollbar-hide justify-center lg:justify-start">
                ...
                <a id="n-detail" onclick="setTab('detail')" class="text-gray-500 hover:text-white pb-2 lg:pb-[18px] lg:pt-5 border-b-2 border-transparent cursor-pointer px-1 flex items-center gap-1">📈 AI 투자상세</a>
                <a id="n-vault" onclick="openPinModal()" class="text-yellow-600 hover:text-yellow-400 pb-2 lg:pb-[18px] lg:pt-5 border-b-2 border-transparent cursor-pointer px-1 flex items-center gap-1 font-black">🔒 AI 지갑</a>
            </nav>
        </div>
            ...
            <div class="h-auto lg:flex-[2.8] bg-panel flex flex-col md:flex-row shrink-0 mt-1 rounded">
                ...
                <div class="w-full md:w-1/2 flex flex-col bg-[#0b1016] h-[400px] lg:h-auto">
                    ...
                    <div class="flex-1 p-2 flex flex-col gap-2 overflow-y-auto border-t border-gray-800 bg-[#0b1016]">
                        <div class="bg-[#1b2029] p-2 rounded border border-gray-700 text-[11px] md:text-xs shrink-0"><div class="text-yellow-400 font-bold border-b border-gray-700 pb-1 mb-1" id="brf-title">[BTC] 실시간 분석</div><div class="flex justify-between mt-1"><span class="text-gray-400">시장 상태:</span><span class="font-bold text-white" id="brf-status">대기</span></div><div class="flex justify-between mt-1"><span class="text-gray-400">RSI:</span><span class="font-bold text-blue-400" id="brf-rsi">0</span></div></div>
                        <div class="bg-[#1b2029] p-2 rounded border border-gray-700 text-[11px] md:text-xs shrink-0"><div class="text-yellow-400 font-bold border-b border-gray-700 pb-1 mb-1">🧠 전체 종합 연산</div><div class="flex justify-between items-center mt-1"><span class="text-gray-400">통합 승률 예측:</span><span class="font-bold text-purple-400 text-sm" id="g-win-rate">0.0%</span></div><div class="flex justify-between items-center mt-1"><span class="text-gray-400">통합 AI 점수:</span><span class="font-bold text-teal-400 text-sm" id="g-ai-score">0 / 100</span></div></div>
                        <div class="bg-[#1b2029] p-2 rounded border border-gray-700 text-[11px] md:text-xs flex-1 flex flex-col min-h-[120px]"><div class="text-white font-bold border-b border-gray-700 pb-1 mb-1 flex items-center gap-1"><span class="text-red-500 animate-pulse">●</span> 개별 연산 랭킹</div><div id="individual-ai-probs" class="flex flex-col gap-1 overflow-y-auto mt-1"><div class="text-center text-gray-500 py-2">수신 대기 중...</div></div></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <main id="v-pnl" class="flex-1 hidden bg-[#0b1016] p-2 md:p-4 overflow-y-auto w-full">
        <div class="max-w-[1200px] mx-auto mt-2 md:mt-4">
            ...
            <div class="bg-panel rounded-lg border border-gray-800 overflow-x-auto w-full mb-8"><div class="flex text-xs text-gray-400 py-3 md:py-4 px-4 md:px-6 border-b border-gray-800 bg-[#0b1016] min-w-[800px]"><div class="w-[20%] font-bold">순위 / 6대 잼</div><div class="w-[20%] text-right">트레이딩 총 실현 손익</div><div class="w-[20%] text-right">승률 (승/패)</div><div class="w-[40%] pl-8">누가 어떤 코인을 샀나?</div></div><div id="ai-ranking-list" class="divide-y divide-gray-800 text-xs md:text-sm min-w-[800px]"></div></div>
    ...
    <main id="v-vault" class="flex-1 hidden bg-[#0b1016] p-2 md:p-4 overflow-y-auto w-full">
        <div class="max-w-[800px] mx-auto mt-2 md:mt-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4" id="vault-agents-list">
                </div>
        </div>
    </main>
    ...
    <script>
        ...
        function verifyPin() {
            if(getEl('pin-input').value === '7777') {
                closePin(); setTab('vault');
            } else { alert('❌ 비밀번호가 틀렸습니다!'); getEl('pin-input').value = ''; }
        }
        ...
        function setTab(t) { 
            ['trd', 'port', 'pnl', 'detail', 'vault'].forEach(id => {
                ...
                if(t === id) { 
                    ...
                    if(n) {
                        n.className = "text-white border-b-2 border-white pb-2 lg:pb-[18px] lg:pt-5 cursor-pointer px-1 font-bold";
                        if(id === 'vault') n.className += " text-yellow-400 border-yellow-400 flex items-center gap-1";
                    }
                } else { 
                    v.className = "hidden"; 
                    if(n) {
                        n.className = "text-gray-500 hover:text-white pb-2 lg:pb-[18px] lg:pt-5 border-b-2 border-transparent cursor-pointer px-1 font-bold"; 
        ...
        function connectWebSocket() {
            ...
            ws.onmessage = e => { 
                try {
                    ...
                    // V40 금고 UI 업데이트
                    if(r.global_vault !== undefined) {
                        if(getEl('vault-total')) getEl('vault-total').innerHTML = `+${Math.round(r.global_vault).toLocaleString()} <span class="text-2xl text-yellow-600">KRW</span>`;
                    }
                    ...
                    // V40 무적 렌더링 블록 (데이터가 없을 때도 뻗지 않음)
                    ...
                    if(r.ranking) {
                        ...
                        r.ranking.forEach((bot, idx) => {
                            ...
                            vaultAgentsHtml += `<div class="bg-[#1b2029] border border-gray-700 rounded-xl p-5 flex flex-col justify-center shadow-lg"><div class="flex justify-between items-center mb-2"><span class="text-white font-bold text-lg">${bot.name}</span><span class="text-xs px-2 py-1 bg-gray-800 text-gray-400 rounded-full">${idx+1}위</span></div><span class="text-2xl font-black text-yellow-500 num-font mt-2">+${vAmt.toLocaleString()} KRW</span><span class="text-[10px] text-gray-500 mt-1">이 AI가 벌어서 안전하게 빼낸 수익</span></div>`;
                        });
...
print("✅ V41: 마스터 금고, 수익 자동 출금 시스템, 0원 표기 버그 완벽 수정!")