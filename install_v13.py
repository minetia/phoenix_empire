...
print("🔥 [Project Phoenix V13] 가짜 과거 데이터 삭제 & 100% 리얼 데이터 연동 중...")
...
# 4. Trader (가짜 과거 데이터 삭제 & 오늘부터의 리얼 데이터만 표시)
...
class PhoenixTrader:
    def __init__(self):
        ...
        self.ai_seed = 100000000.0 # 1억 원 (초기 자본)
    ...
    async def get_portfolio_status(self):
        ...
        # [V13 수정] 가짜 과거 데이터 삭제. 오늘(현재) 기준의 100% 실제 데이터 1줄만 전송
...
# HTML Part 3 (투자손익 디자인 유지, 데이터는 100% 실제 데이터로)
html_part3 = """
    ...
    <script>
        ...
        ws.onmessage = e => { 
            try {
                ...
                // [V13] 일일 투자손익에 가짜 데이터 삭제, 오늘 날짜 실제 데이터만 렌더링
                if(r.daily_pnl) {
                    let dpHtml = "";
...
def install():
    ...
    for path, content in files.items():
        ...
        print(f"✅ V13 리얼 데이터 패치 완료: {path}")
    print("\n🎉 모든 가짜 데이터가 삭제되었습니다! 서버를 다시 실행해주세요.")
